...
启动redis
docker run -d -p 6379:6379  --name redis-6379 redis:3.2 redis-server --port 6379
...
启动sentinel
docker run -d -p 26379:26379 -v /xxx/xxx/sentinel.conf:/usr/local/bin/redis-conf/sentinel.conf --name redis-sentinel-26379  redis:3.2 redis-sentinel /usr/local/bin/redis-conf/sentinel.conf --port 26379
...
--env KAFKA_ZOOKEEPER_CONNECT=10.21.5.4:7998 \
--env KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://10.21.5.4:9092 \
