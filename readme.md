
# [1.language(编程语言)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言))
    readme.md
## [1.java(server端)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端))
    readme.md
### [1.base(基础知识)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识))
    0.概览.md
    1.object(面向对象).md
    2.basicdata(基础数据类型).md
    3.string(字符串).md
    4.keyword(关键字).md
    5.collection(集合类).md
    7.io(流).md
    8.reflect(反射).md
    9.proxy(动态代理).md
    11.annotation(注解).md
    12.jms().md
    13.jmx().md
    15.regex(正则表达式).md
    16.exception(异常).md
    18.api&spi().md
    19.syntacticsugar(语法糖).md
    readme.md
    6.enum(枚举).md
    10.serialize(序列化).md
    14.genericity(泛型).md
    17.time(时间处理).md
    19.浅析java中的语法糖.md
#### [1.object(面向对象)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/1.object(面向对象))
#### [2.basicdata(基础数据类型)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/2.basicdata(基础数据类型))
    refer.md
#### [4.keyword(关键字)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/4.keyword(关键字))
    static.md
    synchronized.md
    voliatile.md
    final.md
#### [5.collection(集合类)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/5.collection(集合类))
    5.4.集合总结.md
    5.2.HashMap源码分析.md
    refer.md
    5.3.ConcurrentHashMap源码分析.md
    5.1.ArrayList、LinkedList、Vector的区别和实现原理.md
##### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/5.collection(集合类)/refer)
#### [6.enum(枚举)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/6.enum(枚举))
    6.1为什么要用枚举实现单例模式（避免反射、序列化问题）.md
#### [7.io(流)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/7.io(流))
    refer.md
#### [8.reflect(反射)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/8.reflect(反射))
    8.1IOC的实现原理—反射与工厂模式的结合.md
#### [9.proxy(动态代理)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/1.base(基础知识)/9.proxy(动态代理))
    9.2java动态代理.md
    9.1静态代理与动态代理.md
### [2.(advance)进阶知识](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识)
#### [1.jvm(Java虚拟机)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/1.jvm(Java虚拟机))
    2.5.内存与线程.md
    2.1.jvm内存模型.md
    2.4.类加载机制.md
    2.3JVM性能监控及故障分析工具.md
    2.2JVM垃圾回收.md
    readme.md
    2.11.jvmargs(jvm参数).md
#### [2.并发](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发)
    readme.md
##### [1.java并发](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发)
    readme.md
###### [01.并发编程的优缺点](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/01.并发编程的优缺点)
###### [02.线程的状态转换以及基本操作](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/02.线程的状态转换以及基本操作)
###### [03.java内存模型以及happens-before规则](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/03.java内存模型以及happens-before规则)
###### [04.彻底理解synchronized](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/04.彻底理解synchronized)
###### [05.彻底理解volatile](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/05.彻底理解volatile)
###### [06.你以为你真的了解final吗？](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/06.你以为你真的了解final吗？)
###### [07.三大性质总结：原子性、可见性以及有序性](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/07.三大性质总结：原子性、可见性以及有序性)
###### [08.初识Lock与AbstractQueuedSynchronizer(AQS)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/08.初识Lock与AbstractQueuedSynchronizer(AQS))
###### [09.深入理解AbstractQueuedSynchronizer(AQS)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/09.深入理解AbstractQueuedSynchronizer(AQS))
###### [10.彻底理解ReentrantLock](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/10.彻底理解ReentrantLock)
###### [11.深入理解读写锁ReentrantReadWriteLock](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/11.深入理解读写锁ReentrantReadWriteLock)
###### [12.详解Condition的await和signal等待通知机制](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/12.详解Condition的await和signal等待通知机制)
###### [13.LockSupport工具](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/13.LockSupport工具)
###### [14.并发容器之ConcurrentHashMap(JDK 1.8版本)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/14.并发容器之ConcurrentHashMap(JDK 1.8版本))
###### [15.并发容器之ConcurrentLinkedQueue](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/15.并发容器之ConcurrentLinkedQueue)
###### [16.并发容器之CopyOnWriteArrayList](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/16.并发容器之CopyOnWriteArrayList)
###### [17.并发容器之ThreadLocal](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/17.并发容器之ThreadLocal)
###### [18.一篇文章，从源码深入详解ThreadLocal内存泄漏问题](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/18.一篇文章，从源码深入详解ThreadLocal内存泄漏问题)
###### [19.并发容器之BlockingQueue](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/19.并发容器之BlockingQueue)
###### [20.并发容器之ArrayBlockingQueue和LinkedBlockingQueue实现原理详解](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/20.并发容器之ArrayBlockingQueue和LinkedBlockingQueue实现原理详解)
###### [21.线程池ThreadPoolExecutor实现原理](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/21.线程池ThreadPoolExecutor实现原理)
###### [22.线程池之ScheduledThreadPoolExecutor](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/22.线程池之ScheduledThreadPoolExecutor)
###### [23.FutureTask基本操作总结](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/23.FutureTask基本操作总结)
###### [24.Java中atomic包中的原子操作类总结](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/24.Java中atomic包中的原子操作类总结)
###### [25.大白话说java并发工具类-CountDownLatch，CyclicBarrier](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/25.大白话说java并发工具类-CountDownLatch，CyclicBarrier)
###### [26.大白话说java并发工具类-Semaphore，Exchanger](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/26.大白话说java并发工具类-Semaphore，Exchanger)
###### [27.一篇文章，让你彻底弄懂生产者--消费者问题](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/27.一篇文章，让你彻底弄懂生产者--消费者问题)
###### [threadlocal](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/1.java并发/threadlocal)
##### [2.juc专项](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/2.juc专项)
    acid.md
    readme.md
    cas原理.md
###### [1.atomic](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/2.并发/2.juc专项/1.atomic)
#### [3.字节码操作](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/3.字节码操作)
    readme.md
##### [1.javassist](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/3.字节码操作/1.javassist)
    readme.md
#### [4.java编译原理](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/2.(advance)进阶知识/4.java编译原理)
    Java的编译原理.md
### [3.Java新特性&修改](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/3.Java新特性&修改)
#### [1.java8](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/3.Java新特性&修改/1.java8)
    1.optional.md
    readme.md
##### [2.lamdba](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/3.Java新特性&修改/1.java8/2.lamdba)
    1.基础介绍
    2.异常处理.md
#### [java13](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/3.Java新特性&修改/java13)
    java13新特性.md
### [4.lib(常用类库)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/4.lib(常用类库))
    readme.md
#### [1.yaml](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/4.lib(常用类库)/1.yaml)
    1.snakeyaml.md
### [5.test(测试)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/1.java(server端)/5.test(测试))
    1.junit().md
    2.mock().md
    3.mockito().md
    4.memorybase(内存数据库).md
    5.Jupiter.md
    7.hamcrest.md
## [2.c(高效语言)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/2.c(高效语言))
    readme.md
## [3.javascript(前端语言)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/3.javascript(前端语言))
    readme.md
## [4.python(编程语言)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/4.python(编程语言))
    readme.md
## [5.shell(脚本语言)](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/5.shell(脚本语言))
    3.日志相关.md
    4.linux防火墙.md
    5.远程执行.md
    6.系统属性相关.md
    readme.md
    1.常用命令.md
    2.权限篇.md
    7.修改机器服务端IP.md
## [6.go](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/6.go)
    readme.md
## [11.(devtool)开发工具](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/11.(devtool)开发工具)
    readme.md
### [1.git](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/11.(devtool)开发工具/1.git)
    readme.md
### [2.gradle](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/11.(devtool)开发工具/2.gradle)
#### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./1.language(编程语言)/11.(devtool)开发工具/2.gradle/refer)
# [2.server(服务端)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端))
    .DS_Store
    readme.md
## [1.solution(解决方案)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案))
    .DS_Store
### [1.微服务](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/1.微服务)
    readme.md
### [3.AWS云平台](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/3.AWS云平台)
    day1.md
    readme.md
    day3.md
    guide.md
    备考.md
    day2.md
### [4.即时通讯](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/4.即时通讯)
    .DS_Store
    readme.md
#### [1.RCS](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/4.即时通讯/1.RCS)
    readme.md
#### [2.MQTT](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/4.即时通讯/2.MQTT)
    readme.md
#### [技术点实现](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/4.即时通讯/技术点实现)
    1.链接类型划分.tny.md
    2.长链接保活方案.tny.md
### [5.音视频](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/5.音视频)
    readme.md
    webrtc.md
### [6.大数据](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/6.大数据)
    readme.md
### [7.安全](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/7.安全)
    web安全.md
    readme.md
### [8.性能调优](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/8.性能调优)
    readme.md
### [10.人工智能](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/10.人工智能)
    人工智能三大基石-数据.md
    人工智能三大基石-算法.md
### [11.数据容灾](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/1.solution(解决方案)/11.数据容灾)
    概述.md
## [2.技术组件](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件)
    readme.md
### [1.spring(应用框架)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/1.spring(应用框架))
    readme.md
#### [0.spring](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/1.spring(应用框架)/0.spring)
    ImportBeanDefinitionRegistrar.md
    BeanDefinitionRegistryPostProcessor.md
#### [1.spring-data-jpa](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/1.spring(应用框架)/1.spring-data-jpa)
    深入介绍.md
    使用说明.md
    1.spring-data-jpa.md
    常用注解.md
#### [2.spring-data-jpa-mybatis](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/1.spring(应用框架)/2.spring-data-jpa-mybatis)
    使用说明.md
#### [3.spring-data-mybatis](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/1.spring(应用框架)/3.spring-data-mybatis)
    使用说明.md
    源码分析.md
### [2.springboot(微服务基础组件)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/2.springboot(微服务基础组件))
    readme.md
#### [1.condition](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/2.springboot(微服务基础组件)/1.condition)
    1.ConditionalOnProperty.md
### [3.springcloud(微服务应用框架)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/3.springcloud(微服务应用框架))
    readme.md
### [4.dubbo(rpc)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/4.dubbo(rpc))
    使用技巧.md
    readme.md
    dubbo-spi.md
    dubbo泛化调用.md
    dubbo路由规则.md
### [5.mybatis](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/5.mybatis)
    0.资源目录.md
    1.学习指南.md
    mybatis多数据源配置.md
#### [源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/5.mybatis/源码分析)
    1.mybatis概览.tny.md
    2.mybatis调试环境搭建.tny.md
    3.mybatis源码目录概览.tny.md
    4.mybatis解析器模块.tny.md
    5.mybatis反射模块.tny.md
    6.mybatis异常模块.tny.md
    7.mybatis数据源模块.md
    8.mybatis事务模块.md
### [6.springcloud-alibaba](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/6.springcloud-alibaba)
    readme.md
### [7.helium](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/7.helium)
    readme.md
### [10.(code)源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析)
    readme.md
#### [1.spring](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/1.spring)
    Spring.md
    spring-mvc.uml
    spring-boot.md
    spring-bean初始化过程.md
    Spring.uml
    spring-transaction.uml
    spring-aop.md
    spring-task.md
    spring-transaction.md
    spring-context.md
    guava-cache.uml
    spring-boot.uml
    guava-cache.md
    spring-mvc.md
##### [flows](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/1.spring/flows)
    guava-cache.vsdx
    spring-task.vsdx
    spring_application_run.gliffy
##### [images](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/1.spring/images)
    local_variable_tables.PNG
    mvc_input_output.PNG
    idea_debug_info.PNG
    debug_info_error.PNG
#### [2.dubbo](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/2.dubbo)
    0.整体概览.md
    1.源码分析指南.md
    2.调试环境搭建.md
    3.项目结构一览.md
    4.API配置（一）之应用.md
    4.API配置(一)之应用.md
    5.API配置(二)之服务提供者.md
    6.API配置(三)之服务消费者.md
    7.属性配置.md
    8.XML配置.md
    9.注解配置.md
    10.外部化配置.md
    10.注册中心(一)之抽象API.md
    11.核心流程一览.md
    12.拓展机制SPI.md
    13.线程池.md
    14.服务暴露(一)之本地暴露(Injvm).md
    15.服务暴露(二)之远程暴露(Dubbo).md
    16.服务引用(一)之本地引用(Injvm).md
    17.服务引用(二)之远程引用(Dubbo).md
    18.Zookeeper客户端.md
    19.注册中心(一)之抽象API.md
    20.注册中心(二)之Zookeeper.md
    21.注册中心(三)之Redis.md
    22.动态编译(一)之Javassist.md
    23.动态代理(一)之Javassist.md
    24.动态代理(二)之JDK.md
    25.动态代理(三)之本地存根Stub.md
    26.服务调用(一)之本地调用(Injvm).md
    27.服务调用(二)之远程调用(Dubbo)[1]通信实现.md
    28.服务调用(二)之远程调用(Dubbo)[2]同步调用.md
    29.服务调用(三)之远程调用(Dubbo)[3]异步调用.md
    30.服务调用（三）之远程调用（HTTP）.md
    31.服务调用（四）之远程调用（Hessian）.md
    32.服务调用（五）之远程调用（WebService）.md
    33.服务调用（八）之远程调用（Redis）.md
    34.服务调用（九）之远程调用（Memcached）.md
    35.调用特性（一）之回声测试.md
    36.调用特性（二）之泛化引用.md
    37.调用特性（三）之泛化实现.md
    38.过滤器（一）之ClassLoaderFilter.md
    39.过滤器（二）之ContextFilter.md
    40.过滤器（三）之AccessLogFilter.md
    41.过滤器（四）之ActiveLimitFilter&&ExecuteLimitFilter.md
    42.过滤器（五）之TimeoutFilter.md
    43.过滤器（六）之DeprecatedFilter.md
    44.过滤器（七）之ExceptionFilter.md
    45.过滤器（八）之TokenFilter.md
    46.过滤器（九）之TpsLimitFilter.md
    47.过滤器（十）之CacheFilter.md
    48.过滤器（十一）之ValidationFilter.md
    49.NIO服务器（一）之抽象API.md
    50.NIO服务器（二）之Transport层.md
    51.NIO服务器（三）之Telnet层.md
    52.NIO服务器（四）之Exchange层.md
    53.NIO服务器（五）之Buffer层.md
    54.NIO服务器（六）之Netty4实现.md
    55.NIO服务器（七）之Netty3实现.md
    56.HTTP服务器.md
    57.序列化（一）之总体实现.md
    57.序列化（二）之Dubbo实现.md
    58.序列化（三）之Kryo实现.md
    59.服务容器.md
    60.集群容错（一）之抽象API.md
    61.集群容错（二）之Cluster实现.md
    62.集群容错（三）之Directory实现.md
    63.集群容错（四）之LoadBalance实现.md
    65.集群容错（五）之Merger实现.md
    66.集群容错（六）之Configurator实现.md
    67.集群容错（七）之Router实现.md
    68.集群容错（八）之Mock实现.md
    69.优雅停机.md
    70.日志适配.md
    71.集成SpringCloud.md
##### [1.问题点汇总](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/2.dubbo/1.问题点汇总)
    1.dubbo-config问题点汇总.md
#### [3.spring-boot](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.(code)源码分析/3.spring-boot)
    11.1.自定义配置源.md
    11.配置加载.md
### [10.应用组件](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件)
#### [1.auth(权限认证)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件/1.auth(权限认证))
##### [1.shiro](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件/1.auth(权限认证)/1.shiro)
    1.Shiro简介.md
    2.Shiro身份验证.md
    3.Shiro授权.md
    4.ShiroInI配置.md
###### [back](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件/1.auth(权限认证)/1.shiro/back)
##### [2.统一认证](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件/1.auth(权限认证)/2.统一认证)
    readme.md
#### [2.日志组件](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/2.技术组件/10.应用组件/2.日志组件)
    readme.md
## [3.分布式](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/3.分布式)
    2.分布式基础理论.md
    readme.md
    1.分布式概念.md
### [1.分布式事务](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/3.分布式/1.分布式事务)
    1.分布式事务概念.md
## [4.技术中间件](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件)
    readme.md
### [1.database(数据库)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库))
    readme.md
    数据库原理.md
#### [关系型数据库](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库)
    readme.md
##### [mysql](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql)
    readme.md
###### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql/refer)
###### [原理分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql/原理分析)
###### [基本使用](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql/基本使用)
###### [常见优化](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql/常见优化)
###### [源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/mysql/源码分析)
##### [tidb](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/关系型数据库/tidb)
    readme.md
#### [图数据库](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/图数据库)
    readme.md
#### [嵌入式数据库](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/嵌入式数据库)
    h2database.md
#### [键值对数据库](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/键值对数据库)
##### [hbase](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/1.database(数据库)/键值对数据库/hbase)
    readme.md
### [2.cache(缓存)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存))
    readme.md
#### [1.基础知识](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存)/1.基础知识)
    readme.md
#### [2.redis](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存)/2.redis)
    1.redis原理与使用.md
    2.redis线程模型.md
    readme.md
##### [cluster](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存)/2.redis/cluster)
    readme.md
##### [code](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存)/2.redis/code)
    readme.md
##### [sentinel](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/2.cache(缓存)/2.redis/sentinel)
    readme.md
    redis-sentinel配置及java示例.md
    Redis哨兵模式与高可用集群.md
### [3.configcenter(配置中心)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/3.configcenter(配置中心))
    readme.md
#### [apollo](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/3.configcenter(配置中心)/apollo)
    readme.md
#### [nacos](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/3.configcenter(配置中心)/nacos)
    readme.md
### [4.logcenter(日志中心)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心))
    readme.md
#### [1.efk使用示例](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/1.efk使用示例)
    efk配置示例.md
    filebeat.yml
##### [res](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/1.efk使用示例/res)
#### [2.java使用示例](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/2.java使用示例)
##### [res](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/2.java使用示例/res)
#### [elasticsearch](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/elasticsearch)
    elasticsearch安装.md
    elasticsearch介绍.md
##### [源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/elasticsearch/源码分析)
#### [filebeat](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/filebeat)
    filebeat介绍.md
    filebeat模块与配置.md
##### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/filebeat/refer)
#### [kibana](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/kibana)
    kibana配置.md
    kibana.md
    kibana安装.md
##### [res](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/kibana/res)
#### [logstash](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/logstash)
    logstash介绍.md
    logstash使用示例kafka2es.md
    logstash安装.md
##### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/4.logcenter(日志中心)/logstash/refer)
### [5.mq(消息队列)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/5.mq(消息队列))
#### [kafka](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/5.mq(消息队列)/kafka)
    readme.md
    概览.tny.md
##### [images](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/5.mq(消息队列)/kafka/images)
##### [源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/5.mq(消息队列)/kafka/源码分析)
### [6.服务注册中心](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/6.服务注册中心)
    服务注册中心对比.md
#### [nacos](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/6.服务注册中心/nacos)
    readme.md
#### [zookeeper](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/6.服务注册中心/zookeeper)
    Zookeeper集群搭建和部署.md
    readme.md
    概览.tny.md
    Zookeeper理论基础.md
    Zookeeper源码.md
    ZAB协议.md
##### [images](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/6.服务注册中心/zookeeper/images)
##### [源码分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/6.服务注册中心/zookeeper/源码分析)
    readme.md
### [7.链路追踪](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/7.链路追踪)
#### [1.SkyWalking](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/7.链路追踪/1.SkyWalking)
    使用指南.md
#### [2.SpringCloudSleuth](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/7.链路追踪/2.SpringCloudSleuth)
    readme.md
### [8.搜索引擎](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/8.搜索引擎)
    readme.md
### [10.nginx](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/10.nginx)
    readme.md
    nginx-tcp.md
    nginx-install.md
### [11.问题诊断](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/4.技术中间件/11.问题诊断)
    arthas.md
## [5.运维](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维)
    readme.md
### [1.容器技术](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术)
    readme.md
#### [1.docker(容器化)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化))
    0.docker概览.md
    1.docker常用命令.md
    readme.md
##### [docker-compose](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/docker-compose)
    readme.md
###### [app](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/docker-compose/app)
##### [docker打包](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/docker打包)
    docker-build-java.md
###### [resources](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/docker打包/resources)
##### [参考示例](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/参考示例)
    数据环境.md
    基础环境.md
    ftp服务器搭建.md
    国内仓库.md
    实例参考.md
    nexus服务搭建.md
##### [常见应用](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/常见应用)
    Docker容器日志管理最佳实践.md
##### [常见问题](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/1.docker(容器化)/常见问题)
    readme.md
#### [2.k8s](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/1.容器技术/2.k8s)
    概览.tny.md
### [2.自动化运维](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维)
    readme.md
#### [1.jenkins](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/1.jenkins)
    readme.md
##### [refer](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/1.jenkins/refer)
#### [2.kubernetes(k8s)](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/2.kubernetes(k8s))
    k8s.md
#### [3.salt](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/3.salt)
    salt_install.md
#### [4.ansible](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/4.ansible)
    readme.md
#### [5.chef](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/5.chef)
    readme.md
#### [6.puppet](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/2.自动化运维/6.puppet)
    readme.md
### [3.统计分析](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/3.统计分析)
    readme.md
### [3.APM](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/3.APM)
    readme.tny.md
### [4.持续集成](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/4.持续集成)
    readme.md
### [5.监控](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/5.监控)
    readme.md
    Zabbix.md
    Prometheus.tny.md
### [6.测试](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试)
#### [AB&灰度&蓝绿测试](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试/AB&灰度&蓝绿测试)
    readme.md
#### [TDD理论](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试/TDD理论)
    readme.md
#### [全链路压测](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试/全链路压测)
    readme.md
#### [单元测试](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试/单元测试)
    readme.md
#### [压力测试](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/6.测试/压力测试)
    ngrinder.md
    readme.md
### [7.虚拟化](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/7.虚拟化)
#### [KVM](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/7.虚拟化/KVM)
    readme.md
#### [OpenVZ](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/7.虚拟化/OpenVZ)
    readme.md
#### [Xen](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/7.虚拟化/Xen)
    readme.md
### [9.云技术](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/9.云技术)
#### [OpenStack](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/9.云技术/OpenStack)
    readme.md
### [10.DevOps](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/10.DevOps)
    readme.md
### [11.文档管理](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/5.运维/11.文档管理)
    readme.md
## [6.中台](https://github.com/wuhaocn/guide-doc/tree/master/./2.server(服务端)/6.中台)
    阿里中台架构.tny.md
# [3.frontend(端)](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端))
    readme.md
## [android](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/android)
    readme.md
## [web](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/web)
### [component(组件集合)](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/web/component(组件集合))
    awsome-js.md
#### [富文本编辑器](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/web/component(组件集合)/富文本编辑器)
    readme.md
##### [tinymce](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/web/component(组件集合)/富文本编辑器/tinymce)
    readme.md
## [跨平台方案](https://github.com/wuhaocn/guide-doc/tree/master/./3.frontend(端)/跨平台方案)
    readme.md
# [4.algorithm(算法)](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法))
    readme.md
## [1.算法基础](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/1.算法基础)
    算法与数据结构基础.md
## [2.数据结构](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构)
### [1.线性表](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/1.线性表)
    线性表简介.md
#### [算法实例](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/1.线性表/算法实例)
    2.两数相加.md
### [2.栈与队列](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/2.栈与队列)
    3.2.1.栈的介绍.md
    3.3.1.优先队列.md
    3.1.2.循环队列.md
    3.1.1.队列介绍.md
    3.1.4.队列和BFS.md
    3.1.3.自带队列示例.md
### [3.数组](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/3.数组)
    树状数组.md
    readme.md
### [4.字符串](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/4.字符串)
    KMP算法.md
    字符串常见算法.md
    动态规划-最长公共子序列LCS及模板.md
### [5.树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树)
#### [1.二叉树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/1.二叉树)
    readme.md
#### [10-LCA](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/10-LCA)
    readme.md
#### [2.完全二叉树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/2.完全二叉树)
    readme.md
#### [3-平衡二叉树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/3-平衡二叉树)
    readme.md
#### [4-二叉查找树（BST）](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/4-二叉查找树（BST）)
    readme.md
#### [5-红黑树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/5-红黑树)
    2.红黑树(五)之Java的实现.md
    readme.md
    0.红黑树原理与介绍.md
    1.红黑树(一)之原理和算法详细介绍.md
#### [6-B-B+B树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/6-B-B+B树)
    readme.md
#### [7-LSM树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/7-LSM树)
    readme.md
#### [8-最小生成树算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/8-最小生成树算法)
    readme.md
#### [9-哈夫曼树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/9-哈夫曼树)
    readme.md
#### [AC自动机](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/AC自动机)
    readme.md
#### [K-D树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/K-D树)
    readme.md
#### [SBT树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/SBT树)
    readme.md
#### [Treap树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/Treap树)
    readme.md
#### [伸展树Splay](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/伸展树Splay)
    readme.md
#### [分块](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/分块)
    readme.md
#### [动态树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/动态树)
    readme.md
#### [可持久化数据结构](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/可持久化数据结构)
    readme.md
#### [字典树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/字典树)
    readme.md
#### [左偏树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/左偏树)
    readme.md
#### [树套树](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/树套树)
    readme.md
#### [树链剖分](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/树链剖分)
    readme.md
#### [点分治](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/点分治)
    readme.md
#### [边分治](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/5.树/边分治)
    readme.md
### [7.图](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/7.图)
#### [17-最短路径算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/7.图/17-最短路径算法)
    readme.md
#### [5-深度优先、广度优先](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/7.图/5-深度优先、广度优先)
    readme.md
### [8.BitSet](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/8.BitSet)
    readme.md
### [9.布隆过滤器](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/2.数据结构/9.布隆过滤器)
    布隆过滤器.md
## [3.常用算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法)
    常见算法对比.md
    目录.md
### [1.概念](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/1.概念)
    概念.md
    1.分治算法.md
    2.动态规划.md
    3.剪枝算法.md
    4.贪心算法.md
    5.回溯算法.md
    6.分支界限法.md
    readme.md
#### [2.动态规划](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/1.概念/2.动态规划)
    8-数字游戏.md
    2.最长公共子序列LCS及模板.md
    4-背包类树形DP.md
    11-插头DP.md
    10-旅行商.md
    7-数位DP.md
    6-矩阵连乘、石子合并.md
    9-状态压缩DP.md
    5-区间DP.md
    3-树形DP.md
    1-线性DP.md
    2-最长公共子序列、最长上升子序列.md
##### [优化](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/1.概念/2.动态规划/优化)
    倍增优化
    单调队列优化
    四边不等式优化
    数据结构优化
    斜率优化
#### [5.回溯算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/1.概念/5.回溯算法)
    10.正则表达式匹配.md
    401.二进制手表.md
    回溯算法概念.md
### [2.排序](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/2.排序)
    9-基数排序.md
    1.冒泡排序.md
    2.插入排序.md
    5-希尔排序.md
    3-快速排序.md
    7.计数排序.md
    6-堆排序.md
    0.选择排序.md
    8-桶排序.md
    4-归并排序.md
    10-Java中的排序工具-TODO.md
### [3.查找](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/3.查找)
    1.Hash表.md
    1.二分查找.md
### [4.推荐算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/4.推荐算法)
    readme.md
### [5.朴素贝叶斯](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/3.常用算法/5.朴素贝叶斯)
    readme.md
## [4.分布式算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/4.分布式算法)
    1.CAP原则.md
    3.0.分布式Paxos.md
    11.一致性哈希算法-1.md
    readme.md
    2.BASE理论.md
    3.1.分布式raft算法.md
    11.一致性哈希算法.md
## [5.人工智能算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/5.人工智能算法)
    readme.md
## [6.加解密算法](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/6.加解密算法)
    SHA1算法分析及实现.md
    算法概要.md
## [7.常用实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现)
### [1.Java实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现/1.Java实现)
    哈希.md
    队列.md
    树.md
    集合.md
    栈.md
### [2.redis实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现/2.redis实现)
    跳跃表.md
### [3.mysql实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现/3.mysql实现)
    树.md
### [4.zookeeper实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现/4.zookeeper实现)
    zab.md
    paxos.md
### [5.kafka实现](https://github.com/wuhaocn/guide-doc/tree/master/./4.algorithm(算法)/7.常用实现/5.kafka实现)
# [5.computer(编程基础知识)](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识))
    readme.md
## [1.design(设计思想)](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想))
    readme.md
### [1.设计模式](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/1.设计模式)
    readme.md
    设计模式六大原则.md
#### [1.创建型](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/1.设计模式/1.创建型)
    1.单例模式.md
    2.简单工厂.md
    3.工厂方法.md
    4.抽象工厂.md
    5.生成器.md
    6.原型模式.md
#### [2.行为型](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/1.设计模式/2.行为型)
    1.责任链.md
    2.命令.md
    3.解释器.md
    4.迭代器.md
    5.中介者.md
    6.备忘录.md
    7.观察者.md
    8.状态.md
    9.策略.md
    10.模板方法.md
    11.访问.md
    12.空对象.md
#### [3.结构型](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/1.设计模式/3.结构型)
    1.适配器.md
    2.桥接.md
    3.组合.md
    4.装饰.md
    5.外观.md
    6.享元.md
    7.代理.md
#### [4.应用场景](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/1.设计模式/4.应用场景)
    Mybatis使用的9种设计模式.md
### [2.MVC](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/2.MVC)
    readme.md
### [3.IOC](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/3.IOC)
    readme.md
### [4.AOP](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/4.AOP)
    readme.md
### [5.UML](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/5.UML)
    readme.md
### [服务设计](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/服务设计)
    readme.md
#### [1.微服务思想](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/服务设计/1.微服务思想)
    readme.md
#### [2.领域驱动](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/1.design(设计思想)/服务设计/2.领域驱动)
    readme.md
## [2.coder](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/2.coder)
    0.readme.md
    1.unicode.md
    2.utf-8.md
## [3.operatingsystem(操作系统)](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统))
    readme.md
### [1-计算机原理](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/1-计算机原理)
    readme.md
### [2-CPU](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/2-CPU)
    readme.md
### [3-进程](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/3-进程)
    readme.md
### [4-线程](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/4-线程)
    readme.md
### [5-协程](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/5-协程)
    readme.md
### [6-Linux](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/3.operatingsystem(操作系统)/6-Linux)
    readme.md
## [4.编程规范](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/4.编程规范)
    代码规范.md
    代码可读性.md
## [10.开发环境](https://github.com/wuhaocn/guide-doc/tree/master/./5.computer(编程基础知识)/10.开发环境)
    update_brew.md
# [6.network(网络)](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络))
    readme.md
## [1.base](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/1.base)
    epoll详解.md
    ipv4&ipv6.md
## [2.coder](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/2.coder)
    SSL重协商攻击.md
## [3.netty](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/3.netty)
    readme.md
    0.Netty源码分析-概览篇.md
### [1.netty-guide](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/3.netty/1.netty-guide)
    1.netty技术点总结.tny.md
### [2.netty-refer](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/3.netty/2.netty-refer)
    1.Future和Promise组件.md
## [4.protocol](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol)
    readme.md
### [1.http](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/1.http)
    0.http概览.md
    1.http方法概述.md
#### [常见问题](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/1.http/常见问题)
    1.跨域问题.md
### [2.mqtt](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/2.mqtt)
    03-ControlPackets.md
    0301-CONNECT.md
    MQTT-3.1.1-CN.html.md
    0310-UNSUBSCRIBE.md
    02-ControlPacketFormat.md
    0307-PUBCOMP.md
    0308-SUBSCRIBE.md
    0311-UNSUBACK.md
    08-AppendixB.md
    0305-PUBREC.md
    06-WebSocket.md
    readme.md
    0303-PUBLISH.md
    0302-CONNACK.md
    0309-SUBACK.md
    00-Contents.md
    00-Preface.md
    01-Introduction.md
    04-OperationalBehavior.md
    0313-PINGRESP.md
    07-Conformance.md
    0314-DISCONNECT.md
    05-Security.md
    0312-PINGREQ.md
    0304-PUBACK.md
    0306-PUBREL.md
#### [images](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/2.mqtt/images)
### [3.rpc](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/3.rpc)
    readme.md
### [4.rtp](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/4.rtp)
    readme.md
### [7.sip](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/4.protocol/7.sip)
    readme.md
## [负载](https://github.com/wuhaocn/guide-doc/tree/master/./6.network(网络)/负载)
    f5访问的真实源IP解决方案.md
# [7.skills(其他技能)](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能))
    .DS_Store
## [1.interview(面试)](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试))
    readme.md
### [algotithm](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/algotithm)
    github.site
### [es](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/es)
    es3.md
    es2.md
    es常见面试题.md
    es1.md
    es4.md
### [index](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/index)
    阿里面试大全.md
    最全技术面试Java：阿里11面试+网易+百度+美团.md
### [kafka](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/kafka)
    kafka常见面试题.md
### [redis](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/redis)
    redis.md
### [zookeeper](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/zookeeper)
    zookeeper常见面试题.md
    zookeeper常见面试题1.md
### [分布式算法](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/分布式算法)
    paxos.md
### [面试集合](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合)
    readme.md
#### [docs](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs)
    index.html
    HomePage.md
    .nojekyll
    _coverpage.md
##### [database](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/database)
    MySQL.md
    MySQL Index.md
    一条sql语句在mysql中如何执行的.md
    MySQL高性能优化规范建议.md
    事务隔离级别(图文详解).md
    一千行MySQL命令.md
###### [Redis](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/database/Redis)
##### [dataStructures-algorithms](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/dataStructures-algorithms)
    几道常见的子符串算法题.md
    Backtracking-NQueens.md
    剑指offer部分编程题.md
    数据结构.md
    几道常见的链表算法题.md
    公司真题.md
    算法学习资源推荐.md
##### [data](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/data)
    java-recommended-books.md
##### [essential-content-for-interview](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/essential-content-for-interview)
    面试必备之乐观锁与悲观锁.md
    简历模板.md
    手把手教你用Markdown写一份高质量的简历.md
###### [BATJrealInterviewExperience](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/essential-content-for-interview/BATJrealInterviewExperience)
###### [MostCommonJavaInterviewQuestions](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/essential-content-for-interview/MostCommonJavaInterviewQuestions)
###### [PreparingForInterview](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/essential-content-for-interview/PreparingForInterview)
##### [github-trending](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/github-trending)
    2018-12.md
    2019-2.md
    2019-6.md
    2019-3.md
    2019-4.md
    2019-1.md
    2019-5.md
    JavaGithubTrending.md
##### [java](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java)
    多线程系列.md
    Java IO与NIO.md
    J2EE基础知识.md
    BIO-NIO-AIO.md
    Java程序设计题.md
    Java编程规范.md
    Java疑难点.md
    Java基础知识.md
###### [Basis](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java/Basis)
###### [Multithread](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java/Multithread)
###### [What's New in JDK8](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java/What's New in JDK8)
###### [collection](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java/collection)
###### [jvm](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/java/jvm)
##### [network](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/network)
    干货：计算机网络知识总结.md
    HTTPS中的TLS.md
    计算机网络.md
##### [operating-system](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/operating-system)
    后端程序员必备的Linux基础知识.md
    Shell.md
##### [system-design](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/system-design)
    设计模式.md
###### [data-communication](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/system-design/data-communication)
###### [framework](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/system-design/framework)
###### [website-architecture](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/system-design/website-architecture)
##### [tools](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/docs/tools)
    Docker-Image.md
    Git.md
    Docker.md
#### [epoll](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/epoll)
    epoll.md
#### [interview_internal](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/interview_internal)
    1.1.4LRU缓存机制.md
    1.2.8对大数据平台中的元数据管理是怎么理解的，元数据收集管理体系是怎么样的，会对大数据应用有什么样的影响.md
    1.1.3给定一个二叉搜索树(BST)，找到树中第K小的节点.md
    1.2.6一颗现代处理器，每秒大概可以执行多少条简单的MOV指令，有哪些主要的影响因素.md
    1.2.4关于并行计算的一些基础开放问题.md
    1.1.1如何实现一个高效的单向链表逆序输出？.md
    1.1.6从innodb的索引结构分析，为什么索引的key长度不能太长.md
    1.3.6假如给你一个新产品，你将从哪些方面来保障它的质量？.md
    1.2.2有一批气象观测站，现需要获取这些站点的观测数据，并存储到Hive中。但是气象局只提供了api查询，每次只能查询单个观测点。那么如果能够方便快速地获取到所有的观测点的数据？.md
    1.3.1最大频率栈.md
    1.3.3如果让你设计一个通用的、支持各种数据库秒级备份和恢复的系统，你会如何设计.md
    1.2.3如何实现两金额数据相加（最多小数点两位）.md
    1.3.7请评估一下程序的执行结果？.md
    1.2.7请分析MaxCompute产品与分布式技术的关系、当前大数据计算平台类产品的市场现状和发展趋势.md
    1.3.0在云计算大数据处理场景中，每天运行着成千上万的任务，每个任务都要进行IO读写.md
    1.3.4如果让你来设计一个支持数据库、NOSQL和大数据之间数据实时流动的数据流及处理的系统，你会考虑哪些问题？如何设计？.md
    1.1.7MySQL的数据如何恢复到任意时间点？.md
    1.2.5请计算XILINX公司VU9P芯片的算力相当于多少TOPS，给出计算过程与公式.md
    1.2.9你理解常见如阿里，和友商大数据平台的技术体系差异以及发展趋势和技术瓶颈，在存储和计算两个方面进行概述.md
    1.1.2已知sqrt(2)约等于1.414，要求不用数学库，求sqrt(2)精确到小数点后10位.md
    1.2.0请解释下为什么鹿晗发布恋情的时候，微博系统会崩溃，如何解决？.md
    1.1.9输入pingIP后敲回车，发包前会发生什么？.md
    9.1.7数据库优化的思路.md
    9.1.2视图的作用，视图可以更改么？.md
    9.1.4索引的工作原理及其种类.md
    9.1.0主键超键候选键外键.md
    10.1.0使用Redis有哪些好处？.md
    10.1.4Memcache与Redis的区别都有哪些？.md
    10.1.1redis相比memcached有哪些优势？.md
    10.1.5Redis常见的性能问题都有哪些？如何解决？.md
    10.1.2redis常见性能问题和解决方案.md
    10.1.3MySQL里有2000w数据，redis中只存20w的数据，如何保证redis中的数据都是热点数据.md
    10.1.8是否使用过Redis集群，集群的原理是什么？.md
    11.5.3在MongoDB中如何排序？.md
    11.4.0为什么要在MongoDB中用Code数据类型？.md
    11.3.9MongoDB支持哪些数据类型？.md
    11.2.8在MongoDB中如何查看数据库列表？.md
    11.2.2非关系型数据库有哪些类型？.md
    11.2.6哪些语言支持MongoDB.md
    11.4.8如何查询集合中的文档？.md
    11.1.4什么是集合？.md
    11.3.8MongoDB支持主键外键关系吗？.md
    11.1.8“mongod”参数有什么？.md
    11.5.4什么是聚合？.md
    11.3.3在MongoDB中如何除去一个数据库？.md
    11.3.1什么是复制？.md
    11.2.7在MongoDB中如何创建一个新的数据库？.md
    11.5.0如何使用AND或OR条件循环查询集合中的文档？.md
    11.2.4在哪些场景使用MongoDB？.md
    11.4.7MongoDB有哪些可替代产品？.md
    11.4.6如何添加索引？.md
    11.2.3为什么用MOngoDB？.md
    11.3.5在MongoDB中如何查看一个已经创建的集合？.md
    11.4.1为什么要在MongoDB中用RegularExpression数据类型？.md
    11.5.6Mongodb存储特性与内部原理.md
    11.4.4“ObjectID”有哪些部分组成？.md
    11.1.9什么是“mongo”？.md
    11.1.1MongoDB是由哪种语言写的？.md
    11.2.1什么是非关系型数据库？.md
    11.2.0MongoDB哪个命令可以切换数据库？.md
    12.1.1zookeeper提供了什么？.md
    12.2.4zk集群下server工作状态.md
    12.2.0zk队列管理.md
    12.1.0zookeeper是什么？.md
    12.2.8zk的session机制.md
    12.1.3zookeeper的四种类型的znode.md
    README.md
    11.4.5在MongoDb中什么是索引？.md
    sync_link
    9.1.8存储过程与触发器的区别.md
    10.1.6redis最适合的场景.md
    11.1.6MongoDB和关系型数据库术语对比图.md
    11.4.3如何在集合中插入一个文档？.md
    12.1.4zookeeper通知机制.md
    12.1.5zookeeper有哪些应用场景？.md
    12.2.2zk中zab的工作原理.md
    12.1.9zk的分布式锁.md
    1.3.5给定一个整数数组和一个整数，返回两个数组的索引，这两个索引指向的数字的加和等于指定的整数。需要最优的算法，分析算法的空间和时间复杂度.md
    1.1.5关于epoll和select的区别，以下哪些说法是正确的.md
    9.1.6数据库范式.md
    9.1.1数据库事务的四个特性及含义.md
    11.1.3什么是数据库？.md
    11.1.5什么是文档？.md
    11.5.5在MongoDB中什么是副本集？.md
    11.4.2为什么在MongoDB中使用ObjectID数据类型？.md
    11.1.0什么是MongoDB？.md
    11.2.9MongoDB中的分片是什么意思？.md
    12.1.2zookeeper文件系统.md
    11.5.1在MongoDB中如何更新数据？.md
    12.2.1zk数据复制.md
    11.1.7什么是“mongod”？.md
    11.2.5MongoDB中的命名空间是什么意思.md
    12.2.3zk是如何保证事物的顺序一致性.md
    1.1.8NFS与SMB的区别？.md
    9.1.5连接的种类.md
    9.1.3drop,delete与truncate的区别.md
    10.1.3zookeeper的四种类型的znode.md
    10.1.7Redis的同步机制了解么？.md
    11.3.6在MongoDB中如何删除一个集合？.md
    11.3.2在MongoDB中如何在集合中插入一个文档？.md
    11.5.2如何删除文档？.md
    11.3.0如何查看使用MongoDB的连接？.md
    11.3.7为什么要在MongoDB中使用分析器？.md
    12.2.6zk同步流程.md
    12.1.7zk的配置管理服务.md
    12.1.6zk的命名服务.md
    12.2.7分布式通知和协调.md
    12.2.5zk是如何选举Leader的？.md
    12.1.8zk的集群管理.md
    github.site
    1.3.2给定一个链表，删除链表的倒数第N个节点，并且返回链表的头结点.md
    1.2.1现有一批邮件需要发送给订阅顾客，且有一个集群（集群的节点数不定，会动态扩容缩容）来负责具体的邮件发送任务，如何让系统尽快地完成发送？.md
    11.3.4在MongoDB中如何创建一个集合？.md
    11.4.9用什么方法可以格式化输出结果？.md
    11.1.2MongoDB的优势有哪些？.md
#### [jvm](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/jvm)
    1.md
#### [集合](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/1.interview(面试)/面试集合/集合)
    5.1.2.2.打破双亲委派模型.md
    5.4.7.0.CAP原则.md
    5.3.2.组合索引的最左优先原则.md
    5.4.9.3.分布式raft算法.md
    5.3.3.1.springmvc请求处理过程.md
    5.1.2.1.类加载.md
    5.2.7.forkjoin.md
    5.2.3.1.NIO详解.md
    5.3.6.1tcp四次挥手.md
    5.2.3.1.bytebuffernettynio.md
    5.1.7.3.class加载.md
    5.1.1.1.内存溢出.md
    5.3.6TCP第四次挥手为什么要等待2MSL.md
    5.3.7get和post请求的区别.md
    5.2.0.1.深拷贝浅拷贝.md
    5.2.3.1.零拷贝原理.md
    5.1.2.3.双亲委派模型概念解释.md
    5.4.9.2.分布式raft算法.md
    5.3.1.1.mysql存储引擎.md
    5.2.3.1.NIO原理.md
    5.2.9syn.md
    5.3.7.1get和post请求的区别.md
    5.2.5spring加载bean的流程.md
    5.4.7.1.base理论.md
    5.1.7.1.class加载.md
    5.3.0.1volatile与内存屏障.md
    5.1.3.2.java动态代理.md
    5.1.1.2.内存溢出.md
    5.2.6java线程池.md
    readme.md
    5.1.7.2.class加载.md
    5.1.3.4.asm字节码操作工具.md
    5.1.0.2.元空间.md
    5.3.8.cookie和session的请求.md
    5.3.4.1.spring事务回滚.md
    5.4.9.1分布式raft算法.md
    5.2.8CountDownLatch和CyclicBarrier的区别.md
    5.3.0.volatile关键字.md
    5.1.9.JAVA的可变类与不可变类.md
    5.1.6.jvm垃圾回收机制.md
    5.3.5.1如何避免脏读，幻读？避免下单重复问题.md
    5.1.3.3.cglib动态代理.md
    5.2.2.2.CAS原理.md
    5.2.9线程加锁有哪些方式synchronized和lock的区别.md
    5.4.6一致性hash算法原理与应用.md
    5.1.3.4.ASM4使用指南.pdf
    5.1.6.3.cms垃圾回收器.md
    5.2.9.Java并发编程：Synchronized底层优化（偏向锁、轻量级锁）.md
    5.1.6.2.垃圾回收器.md
    5.3.6.tcp四次握手断开连接为什么需要TIME_WAIT状态.md
    5.2.1.1.hashmapyuhashtabcunhashmap差别.md
    5.2.2.1CAS.md
    5.1.5.1.jvm状态常用命令.md
    5.1.8.string-stringbuffer-stringbuilder.md
    5.1.0.1.java内存模型.md
    5.1.3.1.动态代理简介.md
    5.1.0.3.Java虚拟机内存模型.java.md
## [2.project(项目)](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/2.project(项目))
    ci-cd.md
## [3.个人及团队成长](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/3.个人及团队成长)
    团队成长.md
    个人成长.md
### [1.团队管理](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/3.个人及团队成长/1.团队管理)
    readme.md
## [4.项目管理](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/4.项目管理)
    readme.md
## [5.政策、法规](https://github.com/wuhaocn/guide-doc/tree/master/./7.skills(其他技能)/5.政策、法规)
    readme.md
# [8.resources](https://github.com/wuhaocn/guide-doc/tree/master/./8.resources)
    readme.md
## [2.技术资源](https://github.com/wuhaocn/guide-doc/tree/master/./8.resources/2.技术资源)
    readme.md
## [3.资讯](https://github.com/wuhaocn/guide-doc/tree/master/./8.resources/3.资讯)
    readme.md
# [9.tool](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool)
## [2.构建工具](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool/2.构建工具)
### [1.gradle](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool/2.构建工具/1.gradle)
    readme.md
### [2.maven](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool/2.构建工具/2.maven)
    maven资源导航.md
    maven介绍.md
### [3.nexus](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool/2.构建工具/3.nexus)
    nexus服务搭建.md
## [3.在线工具](https://github.com/wuhaocn/guide-doc/tree/master/./9.tool/3.在线工具)
    readme.md
# [kbs-source](https://github.com/wuhaocn/guide-doc/tree/master/./kbs-source)
