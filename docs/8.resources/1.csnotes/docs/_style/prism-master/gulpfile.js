...
gulp.task('languages-plugins', function(cb) {
	componentsPromise.then(function(data) {
		...
		for (var p in data.languages) {
			if (p !== 'meta') {
				var title =
					data.languages[p].displayTitle || data.languages[p].title
		...
		tasks.forEach(function(task) {
			var stream = gulp
				...
				.pipe(
					gulp.dest(
						task.plugin.substring(0, task.plugin.lastIndexOf('/'))
					)
				)
...
gulp.task('changelog', function(cb) {
	return gulp
		...
		.pipe(
			replace(/\[[\da-f]+(?:, *[\da-f]+)*\]/g, function(match) {
				return match.replace(
					/([\da-f]{7})[\da-f]*/g,
					'[`$1`](https://github.com/PrismJS/prism/commit/$1)'
				)
			})
		)
		.pipe(gulp.dest('.'))
