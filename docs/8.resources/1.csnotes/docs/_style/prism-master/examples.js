...
;(function() {
	...
	for (var id in languages) {
		...
		;(function(id) {
			...
			language.examplesPath =
				languages.meta.examplesPath.replace(/\{id}/g, id) + '.html'
			...
			fileExists(language.examplesPath).then(function(exists) {
				$u.element.create('label', {
					attributes: {
						'data-id': id,
						title: !exists
							? 'No examples are available for this language.'
							: ''
					},
	...
	function buildContentsHeader(id) {
		...
		if (language.alias) {
			...
			header +=
				'<ul><li><code class="language-none">"language-' +
				id +
				'"</code></li>'
			alias.forEach(function(alias) {
				header +=
					'<li><code class="language-none">"language-' +
					alias +
					'"</code></li>'
			})
			header += '</ul>'
			...
			header +=
				'<p>To use this language, use the class <code class="language-none">"language-' +
				id +
				'"</code>.</p>'
		}
		if (language.require) {
			...
			header +=
				'<p><strong>Dependencies:</strong> The following dependencies need to be loaded before this component: '
			header += require
				.map(function(dep) {
	...
	function update(id) {
		...
		if (language.enabled) {
			if (!language.examplesPromise) {
				language.examplesPromise = getFileContents(
					language.examplesPath
				)
			}
			language.examplesPromise.then(function(contents) {
				...
				loadLanguage(id).then(function() {
					var elements = examples[id].querySelectorAll(
