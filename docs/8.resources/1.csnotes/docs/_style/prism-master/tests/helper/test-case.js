...
module.exports = {
	...
	runTestCase: function(languageIdentifier, filePath) {
		...
		if (null === testCase) {
			throw new Error(
				'Test case file has invalid format (or the provided token stream is invalid JSON), please read the docs.'
			)
		}
		...
		var simplifiedTokenStream = TokenStreamTransformer.simplify(
			compiledTokenStream
		)
		...
		var diff = ''
		while (j < tzd.length) {
			if (exp[i] != tzd[j] || i == exp.length) diff += tzd[j]
			else i++
			j++
		}
