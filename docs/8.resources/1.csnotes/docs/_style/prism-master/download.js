...
;(function() {
	...
	var treeURL =
		'https://api.github.com/repos/PrismJS/prism/git/trees/gh-pages?recursive=1'
	...
	var hstr = window.location.hash.match(
		/(?:languages|plugins)=[-+\w]+|themes=[-\w]+/g
	)
	if (hstr) {
					...
					if (id !== 'meta') {
						if (components[category][id]) {
							if (components[category][id].option !== 'default') {
								if (
									typeof components[category][id] === 'string'
								) {
									components[category][id] = {
										title: components[category][id]
									}
								}
	...
	var qstr = window.location.search.match(
		/(?:languages|plugins)=[-+\w]+|themes=[-\w]+/g
	)
	if (qstr && !hstr) {
	...
	for (var category in components) {
		...
		for (var id in all) {
			...
			var info = (all[id] = {
				...
				enabled: checked,
				require:
					$u.type(all[id].require) === 'string'
						? [all[id].require]
						: all[id].require,
				after:
					$u.type(all[id].after) === 'string'
						? [all[id].after]
						: all[id].after,
				peerDependencies:
					$u.type(all[id].peerDependencies) === 'string'
						? [all[id].peerDependencies]
						: all[id].peerDependencies,
				owner: all[id].owner,
				files: {
					minified: {
			...
			if (!all[id].noJS && !/\.css$/.test(filepath)) {
				info.files.minified.paths.push(
			...
			function getLanguageTitle(lang) {
				if (!lang.aliasTitles) return lang.title
						...
						properties: {
							...
							onclick: (function(id, category, all) {
								return function() {
									...
									if (dependencies[id] && !this.checked) {
										// It’s required by others
										dependencies[id].forEach(function(
											dependent
										) {
											var input = $(
												'label[data-id="' +
				...
				if (category === 'themes') {
					...
					if (themeInput) {
						...
						themeInput.onclick = function() {
							...
							input.onclick()
							themeInputOnclick &&
								themeInputOnclick.call(themeInput)
						}
					}
				}
			})(label)
	...
	form.elements.compression[0].onclick = form.elements.compression[1].onclick = function() {
		minified = !!+this.value
	...
	function update(updatedCategory, updatedId) {
		...
		if (updatedId) {
			...
			$u.element.prop($('label[data-id="' + updatedId + '"] .filesize'), {
				...
				title:
					(updated.js
						? Math.round((100 * updated.js) / updated.all) +
						  '% JavaScript'
						: '') +
					(updated.js && updated.css ? ' + ' : '') +
					(updated.css
						? Math.round((100 * updated.css) / updated.all) +
						  '% CSS'
						: '')
			})
		}
	...
	function getSortedComponents(components, requireName, sorted) {
		...
		while (i < sorted.length) {
			...
			for (var requirement in components[id][requireName]) {
				indexOfRequirement = sorted.indexOf(
					components[id][requireName][requirement]
				)
	...
	function generateCode() {
		...
		for (var category in components) {
			...
			var sorted = getSortedComponentsByRequirements(
				all,
				category === 'languages' ? 'peerDependencies' : 'after'
			)
			...
			for (var i = 0; i < sorted.length; i++) {
				...
				if (info.enabled) {
					...
					info.files[minified ? 'minified' : 'dev'].paths.forEach(
						function(path) {
							if (cache[path]) {
								var type = path.match(/\.(\w+)$/)[1]
		...
		Promise.all([buildCode(promises), getVersion()]).then(function(arr) {
			...
			for (var category in redownload) {
				redownloadUrl +=
					category + '=' + redownload[category].join('+') + '&'
			}
			...
			for (var type in code) {
				...
				$('#download-' + type + ' .download-button').href =
					'data:application/octet-stream;charset=utf-8,' +
					encodeURIComponent(versionComment + '\n' + code[type])
			}
		})
	}
