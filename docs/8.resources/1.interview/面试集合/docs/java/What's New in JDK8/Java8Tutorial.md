...
- [Java 8 Tutorial](#java-8-tutorial)
  ...
  - [Lambda 表达式\(Lambda expressions\)](#lambda表达式lambda-expressions)
  ...
  - [Maps](#maps)
  - [Date API\(日期相关 API\)](#date-api日期相关api)
    - [Clock](#clock)
...
下面的示例展示了将字符串转换为大写字符串。你也可以通过 map 来讲对象转换成其他类型，map 返回的 Stream 类型是根据你 map 传递进去的函数的返回值决定的。
